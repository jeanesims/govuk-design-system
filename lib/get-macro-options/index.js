  ...
  const optionGroups = [
    ...
    .concat(
      nestedOptions.map((option) => {
        const caption = `Options for <code>${option.name}</code>`
        const suffix = option.type === 'array' ? '[].' : '.'

        return {
          // For arrays of nested objects (not strings) append "objects"
          name:
            option.type === 'array' && option.params
              ? `${caption} ${option.type} objects`
              : `${caption} ${option.type}`,

          id: option.slug,

          // Use option name as prefix for nested objects
          // For example, 'text' becomes 'title.text'
          options: option.params
            .map((param) => ({
              ...
              name: ['array', 'object'].includes(option.type)
                ? `${option.name}${suffix}<wbr>${param.name}`
                : param.name
            }))
        }
      })
